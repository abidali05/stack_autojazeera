...
@endif
						 ...
						 <x-input-error :messages="$errors->get('otp')" class="mt-2 text-center" style="color: #FD5631; list-style:none" />
                    </div>
                    ...
                    <div class="col-12 text-center mt-2">
                        <form id="resend-otp-form" method="POST" action="{{ route('resendOtp') }}" style="display: inline;">
                            @csrf
                            <input type="hidden" name="email" value="{{ session('email') }}">
                            <a style="color: #FD5631; cursor: pointer;" onclick="document.getElementById('resend-otp-form').submit();">Resend OTP</a>
                        </form>
                    </div>
                </div>
            </div>
