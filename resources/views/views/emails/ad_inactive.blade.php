...
<body style="font-family: 'Inter', sans-serif; margin: 0; padding: 0;">
    <div style="width: auto; margin: auto; background-color: white; border-radius: 10px; padding: 20px; color:#1F1B2D
 !important">
        ...
        <div style="margin-top: 20px;">
            ...
            We noticed some issues with your ad request, so it wasn’t approved just yet. Don’t worry—it’s a quick fix! <span style="color:#FD5631">Log in to your account to see what needs updating </span>, and as soon as it’s resolved, we’ll activate your ad right away.<br>
Need help? Our team is here for you! Reach out anytime—we’re happy to assist. Contact Us <span style="color:#FD5631">here.</span>
            </p>
					    ...
					    <div style="width:90%; display:flex; background-color:white;border-radius:10px;    margin-right: 17px; border:1px solid #1F1B2D">
                            <div style="background: #fff; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); overflow: hidden; transition: 0.3s ease-in-out; width: 30%; height: 170px; text-align: center;">
                                ...
                                <div style="padding: 10px; width:70%; ">
                                   ...
                                   @endif
                                       <span style="padding: 5px 5px; border-radius: 5px; font-size: 8px; font-weight: bold; display: flex; background-color: {{ $post->condition == 'new' ? '#4581F9' : '#0EB617' }} ; color:#fff; margin-start:5px">
    ...
    padding-top: 2px;">
                                    ...
                                    <div style="display: flex; justify-content: space-between; margin-top: 10px;">
                                        <div style="background-color:#002D690F;width:30%; margin-right: 20px; color:#fff; padding: 5px 10px; border-radius: 10px; text-align: center;">
                                          <i class="{{ $post->mileage_icon }}"></i> 
											  <p style="margin: 5px 0; font-size: 8px; color:#1F1B2D"> {{ $post->milleage ?? 'N/A' }}  KM</p>
                                        </div>
                                        <div style="background-color:#002D690F;width:30%; margin-right: 20px; color:#fff; padding: 5px 10px; border-radius: 10px; text-align: center; ">
                                        <i class="{{ $post->transmission_icon }}"></i>  
											<p style="margin: 5px 0; font-size: 8px; color:#1F1B2D">  {{ ucfirst($post->transmission ?? 'N/A') }}</p>
                                        </div>
                                        <div style="background-color:#002D690F;width:30%; margin-right: 20px; color:#fff; padding: 5px 10px; border-radius: 10px; text-align: center; ">
                                         <i class="{{ $post->fuel_icon }}"></i>  
											<p style="margin: 5px 0; font-size: 8px; color:#1F1B2D">  {{ ucfirst($post->fuel ?? 'N/A') }}  </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <div style="text-align: center; margin-top: 40px;">
            <a href="{{url('ads')}}" style="color: #ffffff; font-size: 12px; font-weight: 500; background-color: #FD5631; padding: 10px 20px; border: none; border-radius: 15px; text-decoration: none;">go  to my account</a>
        </div>
    <div style="margin-top: 20px;">
