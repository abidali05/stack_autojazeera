...
<body style="font-family: 'Inter', sans-serif; margin: 0; padding: 0;">
    <div style="width: auto; margin: auto; background-color: white; border-radius: 10px; padding: 20px;">
        ...
        <div style="margin-top: 20px;">
            ...
            <p style="color: #1F1B2D; font-size: 16px; font-weight: 600;text-align: center;">
            Here are your inactive ads, waiting for your review.
            </p>
        </div>
                    ...
                    @foreach ($chunk as $post)
					     ...
					     <div style="width:90%; display:flex; background-color:#white;border-radius:10px;    margin-right: 17px; border:1px solid black">
                            <div style="background: #fff; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); overflow: hidden; transition: 0.3s ease-in-out; width: 30%; height: 170px; text-align: center;">
                                    <img src="{{ $post->image  }}"   style="max-width: 100%;height: 180px; object-fit: contain; object-position: center; display: block; margin: 0 auto;" alt="Car Image">
                            </div>
     <div style="padding: 10px; width:70%; ">
                                <div style="display: flex; justify-content: space-between; align-items: center;">
                                    <p style="margin:0px; color:#1F1B2D ; font-size:12px;font-weight:600">{{ $post->year ?? 'N/A' }}</p>
...
<!-- <img src="{{ asset('web/images/star-icon.svg') }}" style="margin-bottom: 1px; margin-right: 2px;"> -->                                                Featured
                                            </span>
                                        @endif
                                ...
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 10px;">
                                    <h6 style="margin: 0; font-size: 12px; color: #1F1B2D;">
                                        <i class="bi bi-geo-alt"></i> {{ $post->location->cityname ?? 'Location Not Available' }}
                                    </h6>
                                    <span style="font-size: 8px;padding-top:5px; color: #1F1B2D;    margin-left: auto;">
                                        Last Updated: {{ $post->updated_at ? $post->updated_at->format('F j, Y') : 'N/A' }}
                                    </span>
                                </div>

                                <hr style="margin: 5px 0; border-color: #1F1B2D;">
                                ...
                                <div style="display: flex; justify-content: space-between;">
									...
									<div style="background-color:#002D690F; margin-right: 20px; color:#281F48; padding: 5px 10px; border-radius: 10px; text-align: center;">
										...
										<p style="margin: 5px 0; font-size: 8px;">{{ ucfirst($post->fuel ?? 'N/A') }}</p>
									</div>
								</div>
            ...
            @endforeach
        @else
            <p style="color: #1F1B2D; text-align: center;">No listings available in your city.</p>
        @endif
    </div>


        <div style="text-align: center; margin-top: 40px;">
            <a href="{{url('dashboard')}}" style="color: #ffffff; font-size: 12px; font-weight: 500; background-color: #FD5631; padding: 10px 20px; border: none; border-radius: 25px; text-decoration: none;">Go to my account</a>
        </div>
        ...
        <div style="margin-top: 20px;">
             ...
             <hr style="margin: 15px 0; border-color: #66666680;">
        </div>
        <div style="margin-top: 20px; text-align:center">
			   <p style="color: #1F1B2D; font-size: 14px; font-weight: 300;">
         Any Question? <a hreff="" style="color:#FD5631">Click here <a> to contact us.
            </p>
